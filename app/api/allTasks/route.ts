      ...
      projectItem.forEach((zohoItem: ZohoItemDetail) => {
        ...
        const tasks: TaskDetail[] = data?.itemJObj
          ? Object.entries<any[]>(data.itemJObj).map(([key, value]) => {
              ...
              const durationTime = value[itemProps.durationTime];
              const durationPoint = convertTimeToHours(durationTime);
              ...
              return {
                ...
                durationTime, // khoảng thời gian (duration) estimate
                durationPoint, // khoảng thời gian (duration) estimate tính theo point: 1point = 1h
